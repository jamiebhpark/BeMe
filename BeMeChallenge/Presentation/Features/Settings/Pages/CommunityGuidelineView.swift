...
struct CommunityGuidelineView: View {
    var body: some View {
        List {
            ...
            // ── 위반 시 조치 ─────────────────────────────────
            Section(header: Text("위반 시 조치")) {
                Text("•\t신고가 **10회 누적**되거나 자동 검수(SafeSearch)에서 부적절 판정을 받을 경우, 해당 게시물은 즉시 삭제됩니다.\n\n•\t반복적으로 가이드라인을 위반할 경우 계정이 일시 / 영구적으로 제한될 수 있습니다.")
                    .font(.body)
                    .lineSpacing(4)
                    .padding(.vertical, 4)
            }
